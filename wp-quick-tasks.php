...
add_action( 'admin_enqueue_scripts', 'wp_quick_tasks_enqueue_landing_assets' );
function wp_quick_tasks_enqueue_landing_assets(){
	...
	wp_enqueue_style( 'my-plugin-tailwind', $plugin_url . '/build/tailwind.css');
    wp_enqueue_script('wp-quick-landing-scrips', $plugin_url . '/build/landing.js', $dependencies, $build_asset['version'], true);
}

add_action( 'admin_enqueue_scripts', 'wp_quick_tasks_enqueue_user_assets' );
function wp_quick_tasks_enqueue_user_assets(){
	$page = get_current_screen();
