	...
	function wpqt_set_up_db() {
		...
		if ( WP_QUICKTASKER_DB_VERSION != $wp_quicktasker_db_current_version ) {
			...
			$sql15 = "CREATE TABLE " . TABLE_WP_QUICKTASKER_AUTOMATIONS . " (
				...
				automation_trigger ENUM('task-done', 'task-created', 'task-not-done', 'task-deleted', 'task-assigned') NOT NULL,
				automation_action ENUM('archive-task', 'assign-user', 'new-entity-email', 'deleted-entity-email', 'task-assigned-email') NOT NULL,
