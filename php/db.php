    ...
    function wpqt_set_up_db()
    {
        ...
        if (WP_QUICKTASKER_DB_VERSION != $wp_quicktasker_db_current_version) {
            ...
            $sql15 = 'CREATE TABLE ' . TABLE_WP_QUICKTASKER_AUTOMATIONS . " (
				...
				automation_trigger ENUM('task-done', 'task-created', 'task-not-done', 'task-deleted', 'task-assigned', 'task-unassigned', 'task-public-comment-added', 'task-private-comment-added', 'task-attachment-added', 'task-attachment-deleted', 'woocommerce-order-added', 'seatreg-booking-created', 'seatreg-booking-approved', 'seatreg-booking-pending', 'seatreg-booking-pending-via-manager', 'seatreg-booking-approved-via-manager') NOT NULL,
            ...
            $sql22 = 'CREATE TABLE ' . TABLE_WP_QUICKTASKER_USER_NOTIFICATION_PREFERENCES . " (
				...
				notify_task_completion_changed tinyint(1) NOT NULL DEFAULT 1,
				notify_task_assignment_changed tinyint(1) NOT NULL DEFAULT 1,
				notify_task_archive_changed tinyint(1) NOT NULL DEFAULT 1,
				notify_task_deleted tinyint(1) NOT NULL DEFAULT 1,
				notify_stage_changed tinyint(1) NOT NULL DEFAULT 1,
				notify_due_date_changed tinyint(1) NOT NULL DEFAULT 1,
				notify_comment_added tinyint(1) NOT NULL DEFAULT 1,
				created_at datetime NOT NULL COMMENT 'UTC',
				updated_at datetime NOT NULL COMMENT 'UTC',
				PRIMARY KEY  (id),
